...
export default function StartAssessment() {
  ...
  useEffect(() => {
    ...
    apiFetch(`/aptitude/student/assessments/${id}/start`, { method: "POST" })
      .then((payload) => {
  ...
  const submit = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      await apiFetch(`/aptitude/student/attempts/${data.attempt.id}/submit`, { method: "POST" });
      localStorage.removeItem(getResumeKey(data.attempt.id));
      toast.success("Assessment submitted");
      navigate(`/aptitude/student/results/${data.attempt.id}`);
    } catch (error) {
      toast.error(error.message);
      setSubmitting(false);
    }
  ...
  const syncAttemptTime = useCallback(
    async ({ showErrors = false } = {}) => {
      ...
      try {
        const payload = await apiFetch(`/aptitude/student/attempts/${data.attempt.id}/time`);
        setExtraTimeMinutes(payload.attempt.extra_time_minutes || 0);
  ...
  async function selectAnswer(questionId, option) {
    ...
    try {
      await apiFetch(`/aptitude/student/attempts/${data.attempt.id}/answers`, {
        method: "PUT",
      ...
      <section className="mx-auto max-w-3xl overflow-hidden rounded-md border border-line bg-white shadow-lift">
        ...
        <div className="p-6">
          <div className="grid gap-3 text-sm text-slate-700 md:grid-cols-2">
            <p className="rounded-md bg-surface px-4 py-3 font-semibold">Duration: {effectiveDurationMinutes || data.assessment.duration_minutes} minutes</p>
            <p className="rounded-md bg-surface px-4 py-3 font-semibold">Total Marks: {data.assessment.total_marks}</p>
            <p className="rounded-md bg-surface px-4 py-3 font-semibold">Questions: {data.questions.length}</p>
            <p className="rounded-md bg-surface px-4 py-3 font-semibold">Passing Marks: {data.assessment.passing_marks}</p>
          </div>
  ...
  return (
    <section className="page-stack">
      <div className="page-hero flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            ...
            {extraTimeMinutes ? ` &middot; Extra time ${extraTimeMinutes}m` : ""}
          </p>
        </div>
        <Timer
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_260px]">
          ...
          <div className="mt-6 grid gap-3">
            {Object.entries(question.options).map(([key, value]) => (
              <button
                ...
                className={`flex items-start gap-3 rounded-md border p-4 text-left focus:outline-none focus:ring-2 focus:ring-accent/20 ${
                  selected === key
                    ? "border-accent bg-accent/10 text-accent shadow-sm"
                    : "border-line bg-white text-slate-700 hover:border-line hover:bg-surface"
                }`}
              >
        ...
        <aside className="rounded-md border border-line bg-white p-4 shadow-sm lg:sticky lg:top-28 lg:self-start">
          ...
          <div className="mt-4 grid grid-cols-5 gap-2">
            {data.questions.map((item, index) => (
              <button
                ...
                className={`h-10 rounded-md border text-sm font-bold focus:outline-none focus:ring-2 focus:ring-accent/20 ${
                  index === current
