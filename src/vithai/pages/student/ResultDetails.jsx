...
export default function ResultDetails() {
  ...
  useEffect(() => {
    apiFetch(`/aptitude/student/results/${attemptId}`).then(setData);
  }, [attemptId]);
      ...
      <section className="surface p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {data.topic_analytics.map((topic) => (
            <div key={topic.concept} className="rounded-md border border-line bg-surface/70 p-4">
              <div className="flex justify-between gap-3 text-sm font-semibold">
                <span>{topic.concept}</span>
                <span>{topic.accuracy}%</span>
              </div>
              <div className="mt-3 h-2 rounded bg-slate-100">
                <div className="h-2 rounded bg-accent" style={{ width: `${topic.accuracy}%` }} />
              </div>
              <p className="mt-2 text-xs text-slate-500">
                {topic.correct}/{topic.total} correct &middot; Score {topic.score}
              </p>
            </div>
          ))}
        </div>
      ...
      <section className="space-y-4">
        {data.answers.map((answer, index) => (
          <article key={answer.id} className="rounded-md border border-line bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 rounded-md bg-surface p-4 text-sm text-slate-700">
              <p>
                <span className="font-bold">Selected:</span> {answer.selected_option || "Unanswered"} &middot;{" "}
